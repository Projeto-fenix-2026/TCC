...
document.getElementById('removePhoto').addEventListener('click', () => {
  openModal('Remover foto', 'Deseja remover sua foto de perfil?', async () => {
    const def = 'https://ui-avatars.com/api/?name=Usuario&background=c49a4a&color=fff&size=120';
...
document.getElementById('formEmail').addEventListener('submit', async e => {
  ...
  if (emailNovo !== emailConfirm) { showToast('Os e-mails não conferem.', 'error'); return; }
  ...
  if (ok) { showToast('E-mail atualizado! Verifique sua caixa de entrada.'); document.getElementById('formEmail').reset(); }
});
...
document.getElementById('formSenha').addEventListener('submit', async e => {
  ...
  if (nova !== confirm) { showToast('As senhas não conferem.', 'error'); return; }
  if (nova.length < 8)  { showToast('A senha deve ter pelo menos 8 caracteres.', 'error'); return; }

  const ok = await apiFetch('/perfil/senha', 'PUT', { senha_atual: atual, senha_nova: nova });
  if (ok) {
...
document.getElementById('formDepoimento').addEventListener('submit', async e => {
  ...
  if (!titulo || !texto) { showToast('Preencha o título e o depoimento.', 'error'); return; }
  if (!consent)          { showToast('Você precisa autorizar a publicação.', 'error'); return; }

  const data = await apiFetchData('/depoimentos', 'POST', { titulo, texto, anonimato, visibilidade: vis });
  if (data) {
    depoimentos.unshift({ ...data, titulo, texto, anonimato, visibilidade: vis, criado_em: new Date().toLocaleDateString('pt-BR') });
    renderDepoimentos();
...
async function loadDepoimentos() {
  ...
  if (data) { depoimentos = data; renderDepoimentos(); }
}
...
window.editarDep = function(id) {
  ...
  document.getElementById('formDepoimento').scrollIntoView({ behavior: 'smooth' });
  showToast('Edite e clique em "Publicar" para salvar.', 'info');
};

window.deletarDep = function(id) {
  openModal('Excluir Depoimento', 'Tem certeza? Esta ação não pode ser desfeita.', async () => {
    const ok = await apiFetch(`/depoimentos/${id}`, 'DELETE');
    if (ok) { depoimentos = depoimentos.filter(d => d.id !== id); renderDepoimentos(); showToast('Depoimento excluído.'); }
  });
};
...
async function loadSocorros() {
  ...
  if (data) { socorros = data; renderSocorros(); }
}
...
function renderSocorros() {
  ...
  btnAdd.textContent = socorros.length >= max
    ? (userPlan === 'plus' ? `Limite atingido (${MAX_PLUS}/${MAX_PLUS})` : `Limite gratuito (${MAX_FREE}/${MAX_FREE})`)
    : 'Adicionar número';

  const icons = { phone: '📞', shield: '🛡️', heart: '💜', ambulance: '🚑' };
  grid.innerHTML = socorros.map(s => `
...
window.ligar = numero => { window.location.href = `tel:${numero.replace(/\D/g, '')}`; };

window.deletarSocorro = function(id) {
  openModal('Remover Número', 'Deseja remover este número de socorro?', async () => {
    const ok = await apiFetch(`/socorro/${id}`, 'DELETE');
    if (ok) { socorros = socorros.filter(s => s.id !== id); renderSocorros(); showToast('Número removido.'); }
  });
};
...
document.getElementById('formSocorro').addEventListener('submit', async e => {
  e.preventDefault();
  const max = userPlan === 'plus' ? MAX_PLUS : MAX_FREE;
  if (socorros.length >= max) { showToast('Limite atingido para seu plano.', 'error'); return; }
  ...
  if (!payload.nome || !payload.numero) { showToast('Preencha nome e número.', 'error'); return; }
...
['perfilPublico','exibirFoto','exibirDep','apareceBusca',
 'cookieAnalise','cookiePerso','toggle2fa','alertaLogin'].forEach(id => {
...
document.getElementById('btnDownloadDados').addEventListener('click', async () => {
  showToast('Preparando download…', 'info');
  try {
    const res = await fetch(`${BASE_URL}/perfil/meus-dados`, { headers: authHead() });
    if (res.ok) {
    ...
    } else { showToast('Erro ao gerar arquivo.', 'error'); }
  } catch { showToast('Sem conexão.', 'error'); }
});

document.getElementById('btnDeleteAccount').addEventListener('click', () => {
  openModal('⚠️ Excluir Conta', 'Todos os seus dados serão removidos permanentemente. Tem certeza?', async () => {
    const ok = await apiFetch('/perfil/conta', 'DELETE');
    if (ok) { showToast('Conta excluída.', 'info'); setTimeout(() => (window.location.href = '/'), 2500); }
  });
});
...
document.getElementById('fontPlus').addEventListener('click',  () => { if (fontSize < 150) { fontSize += 10; applyFont(fontSize); localStorage.setItem('fontSize', fontSize); } });
document.getElementById('fontMinus').addEventListener('click', () => { if (fontSize > 70)  { fontSize -= 10; applyFont(fontSize); localStorage.setItem('fontSize', fontSize); } });
...
if (localStorage.getItem('highContrast') === 'true') { document.body.classList.add('high-contrast'); document.getElementById('altoContraste').checked = true; }
if (localStorage.getItem('noAnim')       === 'true') { document.body.classList.add('no-anim');       document.getElementById('reduzirAnim').checked   = true; }
if (localStorage.getItem('dyslexia')     === 'true') { document.body.classList.add('dyslexia');      document.getElementById('fonteDislexia').checked = true; }
...
document.getElementById('btnHistoricoPag').addEventListener('click', async () => {
...
document.getElementById('btnCancelarPlano').addEventListener('click', () => {
  openModal('Cancelar Assinatura', 'Deseja cancelar o plano Plus?', async () => {
    const ok = await apiFetch('/plano/cancelar', 'POST');
    if (ok) { showToast('Assinatura cancelada.'); updatePlanUI('free'); }
  });
});
...
async function apiFetch(endpoint, method = 'GET', body = null) {
  try {
    ...
    return false;
  } catch { showToast('Sem conexão com o servidor.', 'error'); return false; }
}
...
async function apiFetchData(endpoint, method = 'GET', body = null) {
  try {
    ...
    return null;
  } catch { showToast('Sem conexão com o servidor.', 'error'); return null; }
}
...
async function init() {
  ...
  await Promise.all([loadDepoimentos(), loadSocorros(), loadPrivacidade(), loadNotificacoes()]);
}
